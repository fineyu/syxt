...
// 提交企业录入
function submitCompany(name,type,addr,people,desc,img_url,city,uid,phone){
  http.postReq("/api/add-gs", { name:name,id:id,type:type,addr:addr,people:people,desc:desc,img_url:img_url,city:city,uid:uid,phone:phone},function(res){
    if(res.state){
      wx.showToast({
        title: '保存成功',
        icon:'none',
...
Page({
  ...
  submitCompany:function(){
    ...
    if (phone == "") {
      ...
      submitCompany(name, type, addr, people, desc, img_url, city, uid,phone)
    }
  },
  /**
   * 
